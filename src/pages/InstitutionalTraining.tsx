...
import { whoWeServe, institutionalPricingTiers } from "@/data/institutionalPrograms";
...
const iconMap: Record<string, React.ElementType> = { Landmark, Hospital, Globe, Building2, MapPin, ShieldCheck };
...
const InstitutionalTraining = () => {
  ...
  useEffect(() => {
    supabase
      ...
      .select("id, name, short_name, slug, overview, icon, duration_options, is_active, institutional_programs(id)")
      .eq("is_active", true)
      .order("display_order")
      .then(({ data }) => {
        if (data) {
          setCategories(
            data.map((d: any) => ({ ...d, programs: d.institutional_programs || [] }))
          );
        }
      });
  }, []);
  ...
  return (
    <Layout>
          ...
          <div className="max-w-3xl">
            ...
            <div className="mt-8 flex flex-col sm:flex-row gap-4">
              <Button asChild size="lg" className="text-base shadow-lg">
                <a href="#programs">Explore Programs<ArrowRight className="ml-2 h-5 w-5" /></a>
              </Button>
              <Button asChild size="lg" variant="secondary" className="text-base">
                <Link to="/contact"><FileText className="mr-2 h-5 w-5" />Request Institutional Proposal</Link>
              </Button>
            </div>
          </div>
        </div>
      ...
      <section className="py-20 bg-background">
        <div className="container-academy">
          <div className="text-center mb-14">
            <span className="inline-block px-4 py-1.5 rounded-full bg-primary/10 text-primary text-sm font-semibold mb-4">Our Partners</span>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {whoWeServe.map((card, index) => {
              ...
              const serveImages = [serveGovernmentImg, serveHospitalImg, serveInternationalImg, serveAuImg, serveCorporateImg, serveDefenseImg];
              const thumbImg = serveImages[index] || serveGovernmentImg;
              return (
                <Card key={card.title} className="group hover:shadow-lg hover:-translate-y-1 transition-all duration-300 border-border/60 overflow-hidden">
                  <div className="relative h-40 overflow-hidden">
                    <img src={thumbImg} alt={card.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
      ...
      <section id="programs" className="py-20 bg-muted/30">
        <div className="container-academy">
          <div className="text-center mb-14">
            <span className="inline-block px-4 py-1.5 rounded-full bg-teal/10 text-teal text-sm font-semibold mb-4">Training Divisions</span>
            <h2 className="text-3xl lg:text-4xl font-bold text-foreground">Program Categories</h2>
            <p className="mt-4 text-lg text-muted-foreground max-w-2xl mx-auto">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {categories.map((cat) => {
              ...
              return (
                <Card key={cat.id} className="group flex flex-col hover:shadow-xl hover:-translate-y-1 transition-all duration-300 overflow-hidden">
                  <div className="relative h-44 overflow-hidden">
                    <img src={thumbImg} alt={cat.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                    ...
                    <div className="absolute bottom-3 left-4 flex items-center gap-2">
                      ...
                      <h3 className="text-base font-bold text-white drop-shadow-md">{cat.name}</h3>
                    </div>
                  </div>
                  <CardContent className="flex-1 p-6">
                    <p className="text-sm text-muted-foreground leading-relaxed mb-4">{cat.overview}</p>
                    ...
                    <Button asChild className="w-full">
                      <Link to={`/institutional-training/${cat.slug}`}>View Programs<ArrowRight className="ml-2 h-4 w-4" /></Link>
                    </Button>
                  </CardContent>
                </Card>
              );
      ...
      <section className="py-20 bg-background">
        <div className="container-academy">
          <div className="text-center mb-14">
            <span className="inline-block px-4 py-1.5 rounded-full bg-accent/10 text-accent text-sm font-semibold mb-4">Pricing</span>
            <h2 className="text-3xl lg:text-4xl font-bold text-foreground">Institutional Packages</h2>
            <p className="mt-4 text-lg text-muted-foreground max-w-2xl mx-auto">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {institutionalPricingTiers.map((tier) => (
              <Card key={tier.name} className={`flex flex-col transition-all duration-300 hover:shadow-lg ${tier.highlighted ? "ring-2 ring-primary border-primary/30" : "border-border/60"}`}>
                {tier.highlighted && (
                  <div className="bg-primary text-primary-foreground text-xs font-medium text-center py-1.5">Most Popular</div>
                )}
                <CardContent className="flex-1 p-6">
                  <h3 className="text-lg font-bold text-foreground">{tier.name}</h3>
                  <p className="text-sm font-medium text-primary mt-1">{tier.participants}</p>
                  ...
                  <Button asChild className="w-full mt-6" variant={tier.highlighted ? "default" : "secondary"}>
      ...
      <section className="py-16 bg-[hsl(216,50%,16%)]">
        <div className="container-academy text-center">
          <h2 className="text-2xl lg:text-3xl font-bold text-white mb-4">Need a Custom Government Contract?</h2>
          <p className="text-white/70 max-w-xl mx-auto mb-8">
            We design tailored training programs for government agencies, with full compliance documentation and multi-site delivery capabilities.
          </p>
          <Button asChild size="lg" className="text-base">
            <Link to="/contact">Contact Our Institutional Team<ArrowRight className="ml-2 h-5 w-5" /></Link>
          </Button>
        </div>
      </section>
    </Layout>
