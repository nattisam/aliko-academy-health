    ...
    <AdminLayout>
      <div className="space-y-6">
                ...
                <CardContent>
                  ...
                  <Table>
                    ...
                    <TableBody>
                        ...
                        <TableRow key={prog.id}>
                          ...
                          <TableCell>
                            <div className="flex gap-1">
                              <Button variant="ghost" size="icon" onClick={() => openProgDialog(cat.id, prog)}>
                                <Pencil className="h-4 w-4" />
                              </Button>
                              <Button variant="ghost" size="icon" onClick={() => deleteProg(prog.id)}>
                                <Trash2 className="h-4 w-4 text-destructive" />
                              </Button>
                            </div>
                          </TableCell>
                      ...
                      {catProgs.length === 0 && (
                        <TableRow><TableCell colSpan={5} className="text-center text-muted-foreground py-6">No programs yet</TableCell></TableRow>
                      )}
                    </TableBody>
                  </Table>
                </CardContent>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <Input value={catForm.short_name} onChange={(e) => setCatForm({ ...catForm, short_name: e.target.value })} />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
              ...
              <div>
                ...
                <Textarea value={catForm.overview} onChange={(e) => setCatForm({ ...catForm, overview: e.target.value })} />
              </div>
              <div>
                <label className="text-sm font-medium">Target Audience (comma-separated)</label>
                <Input value={catForm.target_audience} onChange={(e) => setCatForm({ ...catForm, target_audience: e.target.value })} />
              </div>
              <div>
                <label className="text-sm font-medium">Duration Options (comma-separated)</label>
                <Input value={catForm.duration_options} onChange={(e) => setCatForm({ ...catForm, duration_options: e.target.value })} />
              </div>
              <div>
                <label className="text-sm font-medium">Delivery Formats (comma-separated)</label>
                <Input value={catForm.delivery_formats} onChange={(e) => setCatForm({ ...catForm, delivery_formats: e.target.value })} />
              </div>
              <div>
                <label className="text-sm font-medium">Certification Types (comma-separated)</label>
                <Input value={catForm.certification_types} onChange={(e) => setCatForm({ ...catForm, certification_types: e.target.value })} />
              </div>
              <div>
                <label className="text-sm font-medium">Ideal For (comma-separated)</label>
                <Input value={catForm.ideal_for} onChange={(e) => setCatForm({ ...catForm, ideal_for: e.target.value })} />
              </div>
              <div className="flex items-center gap-4">
                <div>
                  <label className="text-sm font-medium">Order</label>
                  <Input type="number" className="w-20" value={catForm.display_order} onChange={(e) => setCatForm({ ...catForm, display_order: Number(e.target.value) })} />
                </div>
        ...
        <Dialog open={progDialogOpen} onOpenChange={setProgDialogOpen}>
          <DialogContent>
            <DialogHeader>
              <DialogTitle>{editingProg ? "Edit Program" : "New Program"}</DialogTitle>
            </DialogHeader>
            <div className="space-y-4">
              <div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <Input value={progForm.duration} onChange={(e) => setProgForm({ ...progForm, duration: e.target.value })} />
                </div>
                <div>
                  <label className="text-sm font-medium">Delivery</label>
                  <Input value={progForm.delivery} onChange={(e) => setProgForm({ ...progForm, delivery: e.target.value })} />
                </div>
              </div>
              <div className="flex items-center gap-4">
                <div>
                  <label className="text-sm font-medium">Order</label>
                  <Input type="number" className="w-20" value={progForm.display_order} onChange={(e) => setProgForm({ ...progForm, display_order: Number(e.target.value) })} />
                </div>
                <div className="flex items-center gap-2 pt-5">
                  <Switch checked={progForm.is_active} onCheckedChange={(v) => setProgForm({ ...progForm, is_active: v })} />
                  <span className="text-sm">Active</span>
                </div>
              </div>
